...
public class CharsetCalc_BIG5 implements ICharsetCalc{

 private static Charset mCharset=Charset.forName("BIG5");
    ...
    public double getChance(byte[] pData,int pOffset,int tMaxLen){
        ...
        for(int i=pOffset;i<tActualLen;i++){
            ...
            if(0x8E<=tb2iHead&&tb2iHead<=0xFE){
                ...
                if((0x40<=tb2iLast&&tb2iLast<=0xFE)||(0xA1<=tb2iLast&&tb2iLast<=0xFE)){
                    i++;
                    tCatchByte+=2;
                }
            }
